...
/**
 * Parses an individual Tripadvisor restaurant review page (Stage 2) and maps the
 * extracted fields onto the {@link RestaurantDetail} model. Restaurant pages
 * embed structured data as JSON-LD ({@code application/ld+json}) with a
 * {@code Restaurant}/{@code FoodEstablishment} type, which is the most reliable
 * source for name, address, geo, aggregate rating, review count, price range,
 * cuisine and telephone. DOM selectors are used as a fallback.
 */
@Slf4j
@Component
@ConditionalOnProperty(name = "tripadvisor.crawler.restaurants.enabled", havingValue = "true")
public class RestaurantDetailParser {
    ...
    public RestaurantDetail parse(String html, String url, String sourceListingUrl) {
        Document document = Jsoup.parse(html, url);
        Long tripadvisorId = extractTripadvisorId(url);

        JsonNode ld = findRestaurantJsonLd(document);
        ...
        String cuisine = firstNonBlank(
                cuisineText(ld),
                selectText(document, "span[class*=cuisine]"));
        ...
        log.info("TRIPADVISOR_RESTAURANT_DETAIL_PARSED url={} tripadvisorId={} name='{}' address='{}' locality='{}' country='{}' postalCode='{}' lat={} lon={} rating={} reviewCount={} priceRange='{}' cuisine='{}' phone='{}' descriptionChars={} imageCount={}",
                ...
                detail.priceRange(), detail.cuisine(), detail.phone(),
    ...
    private JsonNode findRestaurantJsonLd(Document document) {
    ...
    private boolean isRestaurantNode(JsonNode node) {
        ...
        return typeStr.contains("Restaurant") || typeStr.contains("FoodEstablishment")
                || typeStr.contains("CafeOrCoffeeShop") || typeStr.contains("BarOrPub");
    }
    ...
    private String cuisineText(JsonNode ld) {
        if (ld == null) {
            return null;
        }
        JsonNode servesCuisine = ld.get("servesCuisine");
        if (servesCuisine == null) {
            return null;
        }
        if (servesCuisine.isTextual()) {
            return clean(servesCuisine.asText());
        }
        if (servesCuisine.isArray()) {
            List<String> items = new ArrayList<>();
            for (JsonNode item : servesCuisine) {
